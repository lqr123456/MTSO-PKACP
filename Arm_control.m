function Tasks=Arm_control(T,dim)
        ...
        for t = 1:T
            Tasks.D(t) = dim;% 设置每个任务的维度
            Tasks.Lb{t} = zeros(1, Tasks.D(t));%下界
            Tasks.Ub{t} = ones(1, Tasks.D(t));%上界
            Amax = task_para(t, 1);%最大角度
            Lmax = task_para(t, 2);%最大长度
            Tasks.Fnc{t} = @(x)fitness_arm(x, Amax, Lmax);% 每个任务的适应度函数
        end
